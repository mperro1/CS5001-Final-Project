...
def password_guesser(target_password: str):
    ...
    for length in range(1, len(target_password) + 1):
        process = multiprocessing.Process(target=password_guess_worker, args=(
            target_password, characters, length, queue))
        process.start()
...
def track_and_record_guess_time(target_password: str):
    ...
    avg_time_per_length = time_taken / \
        (password_length * (password_length + 1) / 2)  # Average time per length
    with open('runtimes.txt', 'a') as file:
        file.write(
            f"Password: {target_password}, Time Taken: {time_taken:.2f}s, Average Time per Character: {avg_time_per_char:.2f}s, Average Time per Length: {avg_time_per_length:.2f}s\n")
...
def main(action: str, length: str = DEFAULT_LENGTH, target_password: str = None):
    ...
    """
    print(
        f"Action: {action}, Length: {length}, Target Password: {target_password}")
    if action == '':
        ...
        if not target_password:
            ...
            guessed_password, time_taken, avg_time_per_char, avg_time_per_length = track_and_record_guess_time(
                target_password)
